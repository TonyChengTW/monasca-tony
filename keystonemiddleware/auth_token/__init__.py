...
class BaseAuthProtocol(object):
    ...
    def process_request(self, request):
        ...
        if request.user_token:
            ...
            try:
                self.log.debug('Authenticating user token-1')
                data, user_auth_ref = self._do_fetch_token(
                    request.user_token,
                    allow_expired=allow_expired)
                self.log.debug('Authenticating user token-2')
                self._validate_token(user_auth_ref,
                                     allow_expired=allow_expired)
                self.log.debug('Authenticating user token-3')
    ...
    def _do_fetch_token(self, token, **kwargs):
        ...
        self.log.warning("_do_fetch_token >> 1")
        if self.kwargs_to_fetch_token:
            self.log.warning("_do_fetch_token >> 2")
            data = self.fetch_token(token, **kwargs)
            self.log.warning("_do_fetch_token >> 3")
        else:
        ...
        self.log.warning("_do_fetch_token >> 4")
        try:
